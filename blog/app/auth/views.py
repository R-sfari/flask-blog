    ...
    if form.validate_on_submit():
        ...
        if user is not None and user.check_password(form.password.data):
            ...
            if redirect_url is None or not redirect_url.startswith('/'):
                redirect_url = url_for('blog.index_posts', _external=True)
...
def confirm(token):
    ...
    if current_user.confirmed:
        return redirect(url_for('.confirmed', _external=True))
...
def delete_user(identifier):
    ...
    return redirect(url_for('.index_users', _external=True))
